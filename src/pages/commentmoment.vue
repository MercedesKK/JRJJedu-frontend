...
import { getCommentMoment, deleteCommentMoment } from '~/api/manager'
...
function getData() {
    getCommentMoment(pageInfo).then(res => {
...
const handleAvatarSuccess = (
    ...
    form.imgUrl = uploadFile.response.data.remoteVideoUrl
...
function handleSubmit() {
    const fun = modalId.value ? updateVideo({ id: modalId.value, name: form.name, videoTypeId: form.videoTypeId, graphicDetails: form.graphicDetails, videoUrl: form.videoUrl }) : createVideo(form)
...
const handleDelete = (id) => {
    messageBox("确定要删除吗？").then(isres => {
        deleteCommentMoment(id).then(res => {
            if (res.code !== 200) {
                toast(res.message, "error");
                return;
            } else {
...
<template>
    <el-card shadow="never" class="border-0">
        ...
        <el-table :data="tableData" stripe style="width: 100%">
            ...
            <el-table-column prop="momentId" label="主题" align="center"></el-table-column>
