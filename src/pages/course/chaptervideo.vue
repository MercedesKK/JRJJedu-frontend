...
import { getChapterVideo, createChapterVideo, deleteChapterVideo, updateChapterVideo,getChapter,getUserData } from '~/api/manager'
...
function getData() {
    getChapterVideo(pageInfo).then(res => {
...
function handleSubmit() {
    const fun = modalId.value ? updateChapterVideo({ id: modalId.value, name: form.name, courseChapterId: form.courseChapterId,videoUrl:form.videoUrl }) : createChapterVideo(form)
...
const handleDelete = (id) => {
    messageBox("确定要删除吗？").then(isres => {
        deleteChapterVideo(id).then(res => {
            if (res.code !== 200) {
                toast(res.message, "error");
                return;
            } else {
...
<template>
    <el-card shadow="never" class="border-0">
        ...
        <el-table :data="tableData" stripe style="width: 100%">
            ...
            <el-table-column prop="courseChapterName" label="章节" align="center"></el-table-column>
        ...
        <el-dialog v-model="addFormDialog" :title="modalTitle" width="30%" 
            ...
            <el-form :model="form"  ref="refForm" label-width="80px">
                ...
                <el-form-item prop="courseChapterId" label="章节">
                    <el-select v-model="form.courseChapterId" placeholder="章节">
